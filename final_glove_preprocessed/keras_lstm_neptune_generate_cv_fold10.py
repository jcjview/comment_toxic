...
kernel_size = 10
cnn_filters = 64
kernel_name = 'lstm_neptune_cv_trans_fold10'
    ...
    while (True):
        for i in range(imax):
            if i * batch_size >= len(y):
                break
            elif (i + 1) * batch_size >= len(y):
                batches = np.arange(i * batch_size, len(y))
            else:
                batches = np.arange(i * batch_size, (i + 1) * batch_size)
            yield x[batches], y[batches]
