...
def load_mnist():
    ...
    train = pd.read_csv(config.TRAIN_DATA_FILE)
    ...
    # data=sequence.pad_sequences(train_tokenized, maxlen=config.MAX_TEXT_LENGTH),
    # test_data=sequence.pad_sequences(test_tokenized,maxlen=config.MAX_TEXT_LENGTH),
    y= train[config.CLASSES_LIST].values

    data, test_data=sequence.pad_sequences(train_tokenized, maxlen=config.MAX_TEXT_LENGTH), sequence.pad_sequences(test_tokenized,
                                                                                            maxlen=config.MAX_TEXT_LENGTH)

    perm = np.random.permutation(len(data))
    idx_train = perm[:int(len(data) * (1 - config.VALIDATION_SPLIT))]
    idx_val = perm[int(len(data) * (1 - config.VALIDATION_SPLIT)):]
...
if __name__ == "__main__":
    ...
    if not args.testing:
        ...
        manipulate_latent(manipulate_model, (x_val, y_val), args)
