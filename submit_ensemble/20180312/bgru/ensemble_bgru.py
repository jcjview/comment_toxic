...
p2 = pd.read_csv('0.9887_bgru1_roll_cv_fold10_500.csv')#9852
...
p4 = pd.read_csv('0.9893_bgru1_cv_fold10_glove_300_prepro_0317_9854.csv')#9854
...
blend[col] = 0.15 * minmax_scale(sup[col].values) + \
             0.15 * minmax_scale(p1[col].values) + \
             0.2 * minmax_scale(p2[col].values) + \
             0.25 * minmax_scale(p3[col].values) + \
             0.25 * minmax_scale(p4[col].values)

print('stay tight kaggler')
blend.to_csv("ensemble_bgru_0319v1.csv.gz", index=False,float_format='%.8f', compression='gzip')#0.9859 0.9862
