...
def train_fit_predict(model, X_train, X_test, y):
    ...
    model.fit(train_x, train_y, batch_size=BATCH_SIZE, epochs=10,  verbose=1,validation_data=(valid_x, valid_y), callbacks=callbacks_list)

    model.load_weights(file_path)
    #model.fit(X_train, y, batch_size=BATCH_SIZE, epochs=2,  shuffle=True,verbose=0)
    return model.predict(X_test)

def submit(y_test):
    sample_submission = pd.read_csv(config.path+"sample_submission.csv")
    sample_submission[CLASSES_LIST] = y_test
    sample_submission.to_csv("cnn_rnn_embeding.csv", index=False)
