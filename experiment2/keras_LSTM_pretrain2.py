...
########################################
def get_X_train_X_test(train_df, test_df,train_toxic):
    ...
    list_sentences_toxic = train_toxic["comment"].fillna("NA").values
    comments = []
    ...
    toxic_comments=[]
    for text in list_sentences_toxic:
        toxic_comments.append(text_to_wordlist(text))
    tokenizer = Tokenizer(num_words=MAX_NB_WORDS)
    tokenizer.fit_on_texts(comments + test_comments)

    sequences = tokenizer.texts_to_sequences(comments)
    test_sequences = tokenizer.texts_to_sequences(test_comments)
    toxic_sequences = tokenizer.texts_to_sequences(toxic_comments)
    ...
    toxic_data = pad_sequences(toxic_sequences, maxlen=MAX_SEQUENCE_LENGTH)
    print('Shape of toxic_data tensor:', toxic_data.shape)

    return toxic_data,tokenizer.word_index
...
prepare embeddings
"""
embedding_matrix_path="lstm_embedding_layer.npz"
...
def get_embedding_matrix(word_index):
    ...
    np.savez(embedding_matrix_path, embedding_matrix)
...
def get_model(word_index):
    ...
    x = Embedding(nb_words, EMBEDDING_DIM, input_length=MAX_SEQUENCE_LENGTH,weights=[embedding_matrix],
                  trainable=False)(input)
    x = Bidirectional(LSTM(50, return_sequences=True, dropout=0.1, recurrent_dropout=0.1))(x)
    x = GlobalMaxPool1D()(x)
    x = Dense(50, activation="relu")(x)
    x = Dropout(0.1)(x)
    x =  Dense(2, activation='sigmoid',name='logist2')(x)
...
def train_fit_predict(model, data,y):
    ...
    test_x = data[0:SPLIT]
    test_y = y[0:SPLIT]
    data_val = data[SPLIT:SPLIT2]
    labels_val = y[SPLIT:SPLIT2]
    data_train = data[SPLIT2:]
    labels_train = y[SPLIT2:]
    ...
    STAMP = 'blstm_pretrain_attack_aggression_%.2f_%.2f' % (rate_drop_lstm, rate_drop_dense)
    print(STAMP)
...
def get_Y(train):
    class_list=['attack','aggression']
    return train[class_list].values

train = pd.read_csv(TRAIN_DATA_FILE)
test = pd.read_csv(TEST_DATA_FILE)
toxic=pd.read_csv('attack_aggression.csv')
toxic_data, word_index = get_X_train_X_test(train, test,toxic)
y=get_Y(toxic)
model=get_model(word_index)
train_fit_predict(model, toxic_data,y)
