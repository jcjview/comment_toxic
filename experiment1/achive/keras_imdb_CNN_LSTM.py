...
from keras.layers import Dense, Input, LSTM, Embedding, Dropout, Conv1D, GlobalAveragePooling1D, merge, PReLU, \
    MaxPooling1D, concatenate, Flatten
...
kernel_name="imdb_cnn_lstm_v1.0"
########################################
...
def get_model(word_index):
    ...
    x=Conv1D(filters=filters,kernel_size=kernel_size,padding='valid',activation='relu',strides=1)(x)
    x=MaxPooling1D(pool_size=pool_size)(x)
    x=Dropout(rate_drop_dense)(x)
    x=LSTM(lstm_output_size,dropout=rate_drop_lstm,recurrent_dropout=rate_drop_lstm)(x)
...
def submit(y_test,bst_val_score,STAMP):
    ...
    sample_submission.to_csv(kernel_name+'%.4f_' % (bst_val_score) + STAMP + '.csv', index=False)
