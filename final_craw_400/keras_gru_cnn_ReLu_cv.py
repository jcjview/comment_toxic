...
from keras.layers import Dense, Input, Embedding, Dropout, Conv1D, GlobalMaxPooling1D, add, GlobalAveragePooling1D, \
    LSTM, BatchNormalization, merge, PReLU, SpatialDropout1D, Bidirectional, GRU, concatenate
...
def get_model(embedding_matrix):
    ...
    x2 = Conv1D(filters=cnn_filters,
                filter_length=kernel_sizes[1],
                padding='same',
                activation='relu'
                )(embedded_sequences)
    # x2 = GlobalMaxPooling1D()(x2)
    x3 = Conv1D(filters=cnn_filters,
                filter_length=kernel_sizes[2],
                padding='same',
                activation='relu'
                )(embedded_sequences)
    # x3 = GlobalMaxPooling1D()(x3)
    x4 = Conv1D(filters=cnn_filters,
                filter_length=kernel_sizes[3],
                padding='same',
                activation='relu'
                )(embedded_sequences)
    xconv=add([x1, x2, x3, x4])
    avg_pool = GlobalAveragePooling1D()(xconv)
    max_pool = GlobalMaxPooling1D()(xconv)
    xconv = concatenate([avg_pool, max_pool])

    units = 128
    xlstm = Bidirectional(GRU(units,  recurrent_dropout=rate_drop_dense,return_sequences=False))(x)#(350, dropout_W=0.2, dropout_U=0.2)(x)
