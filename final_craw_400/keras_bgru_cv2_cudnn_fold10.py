...
from keras.layers import Dense, Input, Embedding, Dropout, Conv1D, GlobalMaxPooling1D, recurrent, RepeatVector, \
    Bidirectional, GRU, BatchNormalization, SpatialDropout1D, CuDNNGRU
...
def get_model(embedding_matrix):
    ...
    x = Bidirectional(CuDNNGRU(lstm_output_size, return_sequences=True))(x)
    x = Dropout(rate_drop_dense)(x)
    x = Bidirectional(CuDNNGRU(lstm_output_size, return_sequences=False))(x)
