...
def dump_X_Y_train_test(train, test, y, word_index, data1, data2, data3):
    ...
    outh5file.create_dataset(name='data1', data=data1)
    outh5file.create_dataset(name='data2', data=data2)
    outh5file.create_dataset(name='data3', data=data3)
    outh5file.close()
...
def load_train_test_y():
    ...
    data1 = outh5file['data1']
    data2 = outh5file['data2']
    data3 = outh5file['data3']
    word_dict = {}
    with open(word_index_path, 'rb') as f:
        word_dict = pickle.load(f)
    return train, test, y, word_dict, data1, data2, data3
...
def get_X_train_X_test(train_df, test_df, train1, train2, train3):
    ...
    list_sentences_train1 = train1["comment_text"].fillna("NA").values
    list_sentences_train2 = train2["comment_text"].fillna("NA").values
    list_sentences_train3 = train2["comment_text"].fillna("NA").values
    comments1 = list(list_sentences_train1)
    comments1 = multi_preprocess(job=4, comments=comments1)
    sequences1 = tokenizer.texts_to_sequences(comments1)
    data1 = pad_sequences(sequences1, maxlen=MAX_TEXT_LENGTH)

    comments2 = list(list_sentences_train2)
    comments2 = multi_preprocess(job=4, comments=comments2)
    sequences2 = tokenizer.texts_to_sequences(comments2)
    data2 = pad_sequences(sequences2, maxlen=MAX_TEXT_LENGTH)

    comments3 = list(list_sentences_train3)
    comments3 = multi_preprocess(job=4, comments=comments3)
    sequences3 = tokenizer.texts_to_sequences(comments3)
    data3 = pad_sequences(sequences3, maxlen=MAX_TEXT_LENGTH)

    print('Shape of test_data tensor:', test_data.shape)

    return data, test_data, tokenizer.word_index, data1, data2, data3
